# To check Web.Config for Authorized Types
# http://support.microsoft.com/default.aspx?scid=kb;EN-US;968793
# Rule to look for missing authorized types in the web.config
# Rule ID: 1204
#
# Created: 8/22/2012
...
function FindMissingTypes{
	...
	$missing_type_found=$false
	$authTypes_xml = [xml]'<authorizedTypes>
	...
	$authTypesNode = $authTypes_xml.authorizedTypes
	$authTypeNodes = $authTypesNode.selectNodes("authorizedType")
	foreach ($authtypenode in $authTypeNodes){
	    $auth_type_found=$false
		if (Test-Path "$path\web.config"){
			#Make a backup of the web config.
			(Get-Content "$path\web.config") | Out-File "$path\web.config.bak"
			$xml = [xml](Get-Content "$path\web.config")
			$parent_node = $xml.SelectSingleNode("/configuration/System.Workflow.ComponentModel.WorkflowCompiler/authorizedTypes")
			$nodes = $xml.SelectNodes("/configuration/System.Workflow.ComponentModel.WorkflowCompiler/authorizedTypes/authorizedType")
			foreach ($node in $nodes){
				if ([System.String]::Compare($node.OuterXml, $authtypenode.OuterXml,$true) -eq 0){
					$auth_type_found =$true
					break
				}
			}
			if ($auth_type_found -ne $true){
				$missing_type_found=$true
				$oNode = $parent_node.OwnerDocument.ImportNode($authtypenode,$true)

				#add missing type to the web config.
				$parent_node.AppendChild($oNode)
				$xml.save("$path\web.config")
			}			
		}
	}

	return $missing_type_found
}

Import-LocalizedData -BindingVariable LocalizedMessages 
Write-DiagProgress -Activity $LocalizedMessages.PROGRESSBAR_ID_CRM_MISSINGAUTHTYPES_CHECK -Status $LocalizedMessages.PROGRESSBAR_ID_CRM_MISSINGAUTHTYPES_CHECKDesc
"--> Entered TS_MissingWebConfigAuthTypes.ps1" | WriteTo-StdOut
$Error.Clear()
$RootCauseDetected=$false

# Need to check this for CRM 4.0 only as CRM 2011 is only supported on Windows Server 2008 
if (Test-Path HKLM:\Software\Microsoft\MSCRM){
	...
	if ($MSCRMKey -ne $null){
		...
		if ($server_version -eq "4"){ # Only run this rule for CRM 4.0
			#Get web.config location from registry
			$website_path = $MSCRMKey.GetValue("WebSitePath")
			if ($website_path -ne $null -and $website_path -ne [System.String]::Empty){
				$RootCauseDetected = FindMissingTypes $website_path
...
{
	$SupportTopicsID = "31508834"
	Write-GenericMessage -RootCauseID "RC_MissingWebConfigAuthTypes" -Verbosity "Error" -SolutionTitle "Web.Config May Be Missing Authorized Types After Installation or Removal of CRM 4.0 Update" -PublicContentURL "http://support.microsoft.com/default.aspx?scid=kb;EN-US;968793"  -ProcessName "W3WP.exe" -SupportTopicsID $SupportTopicsID 
	Update-DiagRootCause -Id "RC_MissingWebConfigAuthTypes" -Detected $true 
	if ($Error.Count -gt 0)
	{
	...
	Update-DiagRootCause -Id "RC_MissingWebConfigAuthTypes" -Detected $false 
}
"<-- Exited TS_MissingWebConfigAuthTypes.ps1" | WriteTo-StdOut
# SIG # Begin signature block
# MIInwwYJKoZIhvcNAQcCoIIntDCCJ7ACAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUAB